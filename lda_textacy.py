...
#            print ('=Handling competence ' + comp_name)
            comp_text = ''
            for filename in glob.glob(comp_path + '/*.txt'):
                clean_text = preprocess_text(open(filename, 'r').read().decode('utf-8'), no_urls=True, no_emails=True, 
                                    no_phone_numbers=True, no_numbers=True, no_currency_symbols=True, 
                                    no_punct=True, no_contractions=True, no_accents=True)
...
#                comp_text += open(filename, 'r').read().decode('utf-8')
            ...
            texts.append(doc.to_terms_list(named_entities=False, as_strings=True))
    return texts
...
#    doc_term_matrix = vectorizer.fit_transform(build_comp_termlist())
    doc_term_matrix, id2term = vsm.doc_term_matrix(build_comp_termlist())#, weighting='tfidf', normalize=True, smooth_idf=True, 
...
#    print(repr(doc_term_matrix))
    ...
    for topic_idx, top_docs in model.top_topic_docs(doc_topic_matrix, topics=[0,1], top_n=2, weights=True):
        print(str(topic_idx) + '\t' + str(top_docs))
#        for j in top_docs:
#            print(corpus[j].metadata['title'])
    model.termite_plot(doc_term_matrix, id2term, topics=-1,  n_terms=40, sort_terms_by='seriation', save='./lda-topics.png')
